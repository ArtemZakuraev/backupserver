...
class S3Config(Base):
    ...
    # Связи
    backup_tasks = relationship("BackupTask", back_populates="s3_config")
...
class BackupTask(Base):
    ...
    agent_id = Column(Integer, ForeignKey("agents.id"), nullable=False)
    s3_config_id = Column(Integer, ForeignKey("s3_configs.id"), nullable=False)
    ...
    s3_config = relationship("S3Config", back_populates="backup_tasks")
...
class PostgresBackupTask(Base):
    ...
    # Связи
    s3_config = relationship("S3Config")
